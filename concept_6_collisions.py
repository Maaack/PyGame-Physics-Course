...
class MyCircle:
    ...
    def add_drag(self, time):
        self.velocity -= self.velocity * drag * time
        return self.velocity

    # v(n+1) = v(n) + at
    def velocity_and_accel(self, time):
        return self.velocity + self.accel * time

    # x(n+1) = x(n) + tv(n) + 1/2at^2
    def position_and_vector(self, time):
        x = self.x + (self.velocity.x * time) + 0.5*(self.accel.x * (time ** 2))
        y = self.y + (self.velocity.y * time) + 0.5*(self.accel.y * (time ** 2))
        return x, y
...
pygame.display.set_caption('Concept 6 - Collisions')
